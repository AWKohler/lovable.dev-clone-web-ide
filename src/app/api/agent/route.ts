...
export async function POST(req: Request) {
  try {
    ...
    const systemPromptWeb = [
      ...
      "```",
      "",
      "**The typed `api` object and document types come from `_generated`, using a path relative to the IMPORTING FILE:**",
      "```typescript",
      "// ✅ CORRECT — importing from /src/App.tsx (one level below root)",
      "import { api } from '../convex/_generated/api';",
      "import { Id, Doc } from '../convex/_generated/dataModel';",
      "",
      "// ✅ CORRECT — importing from /src/components/Foo.tsx (two levels below root)",
      "import { api } from '../../convex/_generated/api';",
      "import { Id, Doc } from '../../convex/_generated/dataModel';",
      "```",
      "",
      "**What `_generated` contains after deploy:** `api.d.ts`, `api.js`, `dataModel.d.ts`, `server.d.ts`, `server.js`.",
      "There is NO `react.ts`, `react.d.ts`, or `react.js` in `_generated` — never import hooks from there.",
      "",
      "**Common wrong patterns — never write these:**",
      "```typescript",
      "// ❌ WRONG — _generated/react does not exist",
      "import { useQuery } from './convex/_generated/react';",
      "import { useQuery } from '../convex/_generated/react';",
      "",
      "// ❌ WRONG — path depth must match the importing file's location, not always '../'",
      "import { api } from './convex/_generated/api';   // wrong from inside /src",
      "import { api } from 'convex/_generated/api';     // wrong (no leading dots)",
      "```",
      "",
      ...
      "   - \"[plugin:vite:import-analysis] Failed to resolve import '...convex/_generated/api'\" → wrong relative depth; count the slashes from the importing file to the project root and use that many `../`",
      "",
      ...
      "- **For Convex projects: `useQuery`/`useMutation`/`useAction` are ALWAYS imported from `'convex/react'` (npm package). NEVER from `_generated/react` (that file doesn't exist).**",
      "- **For Convex projects: `api` is imported from a `_generated/api` path that is relative to the importing file's depth below root.**",
      "",
      "---",
      "",
