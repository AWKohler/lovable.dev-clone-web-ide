...
export function SupabasePicker({
      ...
      .then((data) => {
        ...
        const mapped: Org[] = Array.isArray(data)
          ? data.map((o: Record<string, unknown>) => ({
              id: String(o.id || o.uuid || o.slug || o),
              name: String(o.name || o.slug || "Organization"),
  ...
  const loadProjects = async (orgId: string) => {
    ...
    try {
      const res = await fetch(
        `/api/supabase/projects?org_id=${encodeURIComponent(orgId)}`,
      );
  ...
  const pollForNewProject = async (org: Org, attemptsLeft: number) => {
    if (!org.id || attemptsLeft <= 0) {
      toast({
        title: "Not detected",
        description:
          "New Supabase project was not detected. You can select it from the list once it appears.",
      });
      return stopPolling();
    }
    try {
      const res = await fetch(
        `/api/supabase/projects?org_id=${encodeURIComponent(org.id)}`,
        { cache: "no-store" },
      );
      const data = await res.json();
      const mapped: Project[] = Array.isArray(data)
          ...
          if (projectId) {
            try {
              ...
              toast({
                ...
                description: `${candidate?.name || ref} is now connected.`,
              });
            } catch (e) {
              console.error("Auto-link Supabase project failed", e);
              toast({
                title: "Link failed",
                description: "Could not link the new project automatically.",
              });
            }
          }
          onSelected?.(ref);
    ...
    pollRef.current = window.setTimeout(
      () => pollForNewProject(org, attemptsLeft - 1),
      4000,
    );
  };
  ...
  const handlePick = async (org: Org, proj: Project) => {
    ...
    if (projectId) {
      try {
        ...
        toast({
          title: "Supabase linked",
          description: `${proj.name || ref} is now connected.`,
        });
      } catch (e) {
        console.error("Failed to connect Supabase project", e);
        toast({
          title: "Link failed",
          description: "Could not link the selected project.",
        });
      }
    }
    onSelected?.(ref);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch(
          `/api/supabase/link?projectId=${encodeURIComponent(projectId)}`,
          { cache: "no-store" },
        );
        const data = await res.json();
  ...
  return (
    <div ref={rootRef} className={cn("relative", className)}>
      <button
        onClick={() => setOpen((v) => !v)}
        className={cn(
          "inline-flex items-center gap-2 rounded-full px-3 py-1.5 text-sm font-medium border hover:border-transparent transition",
          selectedName
            ? "bg-neutral-900 text-[var(--sand-text)] border-neutral-900"
            : "border-border bg-elevated text-[var(--sand-text)] shadow hover:bg-accent/15",
        )}
        title="Connect Supabase"
      >
        <Database
          className={cn("h-4 w-4", selectedName ? "text-emerald-400" : "")}
        />
            ...
            {!loading &&
              orgs.map((o) => (
                <div
                  ...
                  onMouseLeave={() =>
                    setHoverOrg((prev) => (prev === o.id ? null : prev))
                  }
                >
