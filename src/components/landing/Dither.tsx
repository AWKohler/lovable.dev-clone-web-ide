...
const fallbackBase = new THREE.Color("#fefefe");
const fallbackAccent = new THREE.Color("#1d52f1");

const toRgbTuple = (color: THREE.Color): [number, number, number] => [
  color.r,
  color.g,
  color.b,
];

const getCssVarColor = (
  variable: string,
  ...
  try {
    const parsed = new THREE.Color(value);
...
const useSandColors = () => {
  const baseFallback = useMemo(() => toRgbTuple(fallbackBase), []);
  const accentFallback = useMemo(() => toRgbTuple(fallbackAccent), []);
  ...
  useEffect(() => {
    const update = () => {
      setColors({
        base: getCssVarColor("--sand-bg", baseFallback),
        accent: getCssVarColor("--sand-accent", accentFallback),
      });
    };
    update();
...
export default function Dither({
  ...
  const sandColors = useSandColors();
  const resolvedBaseColor = useMemo<[number, number, number]>(() => {
    if (!useSandTheme) return baseColor ?? toRgbTuple(fallbackBase);
    return baseColor ?? sandColors.base;
  }, [baseColor, sandColors.base, useSandTheme]);

  const resolvedWaveColor = useMemo<[number, number, number]>(() => {
    if (!useSandTheme) return waveColor ?? toRgbTuple(fallbackAccent);
    return waveColor ?? sandColors.accent;
  }, [sandColors.accent, useSandTheme, waveColor]);
